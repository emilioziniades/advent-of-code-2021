...
func getStateNeighbours(currentState *State) map[*State]int {
	...
	for podPosition, podType := range *currentState {
		...
		nextPodPositions := getPodNextPositions(podPosition, podType, currentState)
...
func getPodNextPositions(podPosition Point, podType rune, state *State) map[Point]int {
	...
	if podPosition.IsHome(podType) && !podHomeButMustMakeSpace(podPosition, podType, state) {
		// home already, nowhere to go
		return nextPositions

		// TODO : a pod could be home, but has to let another pod type out
	}
...
func podHomeButMustMakeSpace(podPosition Point, podType rune, state *State) bool {
	// if
	// - pod is home
